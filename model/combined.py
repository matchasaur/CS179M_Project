...
def start_load(m, operations, bay_containers):
    manifest = open('updated_manifest.txt', 'r')
    targets = []
    if bay_containers != None:
        for i in range(len(bay_containers)-1):
...
def write_manifest(path, tuples):
    ...
    if bay_containers is not None:
        start_load(updated_manifest, operations, bay_containers)

    print(bay_containers)
    print("Manifest updated.")
